...
function moneyRandom() {
    ...
    if (e.length > 0) {
        var t = parseFloat(e.html().replace("$", "").replace(",", ".")), n = Math.random() * 10, r = 1;
        n >= 5 && (r = -1);
...
$(document).ready(function() {
    if ($(".usertable").length > 0) {
        var e = {sPaginationType: "full_numbers", oLanguage: {sSearch: "<span>Search:</span> ", sInfo: "Showing <span>_START_</span> to <span>_END_</span> of <span>_TOTAL_</span> entries", sLengthMenu: "_MENU_ <span>entries per page</span>"}, sDom: "lfrtip", aoColumnDefs: [{bSortable: !1, aTargets: [0, 5]}], oColVis: {buttonText: "Change columns <i class='icon-angle-down'></i>"}, oTableTools: {sSwfPath: "js/plugins/datatable/swf/copy_csv_xls_pdf.swf"}}, t = $(".usertable").dataTable(e);
        $(".dataTables_filter input").attr("placeholder", "Search here...");
        $(".dataTables_length select").wrap("<div class='input-mini'></div>").chosen({disable_search_threshold: 9999999});
        $("#check_all").click(function(e) {
            $("input", t.fnGetNodes()).prop("checked", this.checked)
        });
        $.datepicker.setDefaults({dateFormat: "dd-mm-yy"});
        t.columnFilter({sPlaceHolder: "head:after", sRangeFormat: "{from}{to}", aoColumns: [null, {type: "text"}, {type: "text"}, {type: "select", bCaseSensitive: !0, values: ["Active", "Inactive", "Disabled"]}, {type: "date-range"}, null]});
    ...
    if ($(".username-check").length > 0) {
        ...
        $.mockjax({url: "/check", contentType: "text/json", response: function(e) {
                this.responseText = {available: "true"};
                e.data.username == "" && (this.responseText = {available: "false"})
            }})
    }
    if ($("#user").length > 0) {
        $.mockjaxSettings.responseTime = 500;
        ...
        $.mockjax({url: "/groups", response: function(e) {
                this.responseText = [{value: 0, text: "Guest"}, {value: 1, text: "Service"}, {value: 2, text: "Customer"}, {value: 3, text: "Operator"}, {value: 4, text: "Support"}, {value: 5, text: "Admin"}]
            }})
    }
    ...
    $("#message-form").submit(function(e) {
        e.preventDefault();
        var t = $(this), n = new Array("Lorem ipsum incididunt dolor...", "Lorem ipsum velit in incididunt id consectetur commodo.", "Lorem ipsum voluptate dolore occaecat reprehenderit anim elit nostrud.", "Lorem ipsum in dolor Excepteur et non sunt elit non officia in qui deserunt cupidatat aliquip."), r = t.find("input[type=text]").val(), i = t.parents(".messages");
        if (t.find("button").attr("disabled") == undefined) {
            var s = i.find(".right").first().clone(), o = i.find(".left").first().clone();
    ...
    if ($("#online-users").length > 0) {
        ...
        n.on("resizestop", function() {
            $("#online-users").show().sparkline(getUser(), {width: n.width() > 200 ? 100 : n.width() - 100, height: "25px", enableTagOptions: !0});
            $("#balance").show().sparkline(getBalance(), {height: "25px", barWidth: n.width() > 200 ? 4 : Math.floor((n.width() - 100) / 17) - 1, enableTagOptions: !0})
        })
    }
    var r = [];
    ...
    var u = new Array;
    u.push({label: "Clicks per month", data: r, bars: {show: !0, barWidth: .2, order: 1, lineWidth: 2}});
    u.push({label: "Referalls per month", data: s, bars: {show: !0, barWidth: .2, order: 2}});
    u.push({label: "Downloads per month", data: o, bars: {show: !0, barWidth: .2, order: 3}});
    if ($("#flot-spider").length > 0) {
        var r = [[0, 40], [1, 50], [2, 80], [3, 70], [4, 60]], s = [[0, 60], [1, 45], [2, 60], [3, 60], [4, 95]], o = [[0, 50], [1, 40], [2, 50], [3, 95], [4, 30]];
        c = [{label: "Pies", color: "green", data: r, spider: {show: !0, lineWidth: 12}}, {label: "Apples", data: s, spider: {show: !0}}, {label: "Cherries", data: o, spider: {show: !0}}];
        $.plot($("#flot-spider"), c, {series: {spider: {active: !0, spiderSize: .8, legs: {data: [{label: "OEE"}, {label: "MOE"}, {label: "OER"}, {label: "OEC"}, {label: "Quality"}], font: "14px Open Sans"}}}, grid: {mode: "spider"}})
    }
    if ($("#flot-radar").length > 0) {
        var r = [[0, 40], [1, 50], [2, 80], [3, 70], [4, 60]], s = [[0, 60], [1, 45], [2, 60], [3, 60], [4, 95]], o = [[0, 50], [1, 40], [2, 50], [3, 95], [4, 30]];
        c = [{label: "Pies", color: "green", data: r, spider: {show: !0, lineWidth: 12}}, {label: "Apples", data: s, spider: {show: !0}}, {label: "Cherries", data: o, spider: {show: !0}}];
        $.plot($("#flot-radar"), c, {series: {spider: {active: !0, spiderSize: .8, legs: {data: [{label: "OEE"}, {label: "MOE"}, {label: "OER"}, {label: "OEC"}, {label: "Quality"}], font: "14px Open Sans"}}}, grid: {mode: "radar"}})
    }
    if ($("#flot-2").length > 0) {
        var a = [], f = [];
        ...
        var l = $.plot($("#flot-2"), [{data: a, label: "sin(x)"}, {data: f, label: "cos(x)"}], {series: {lines: {show: !0}, points: {show: !0}}, grid: {hoverable: !0, clickable: !0}, yaxis: {min: -1.2, max: 1.2}})
    }
    if ($("#flot-5").length > 0) {
        var c = [], h = Math.floor(Math.random() * 4) + 2;
        for (var i = 0; i < h; i++)
            c[i] = {label: "Series" + (i + 1), data: Math.floor(Math.random() * 100) + 1};
        $.plot($("#flot-5"), c, {series: {pie: {show: !0}}});
        $.plot($("#flot-6"), c, {series: {pie: {show: !0, radius: 1, label: {show: !0, radius: 1, formatter: function(e, t) {
        ...
        $.plot($("#flot-7"), c, {series: {pie: {show: !0, radius: 1, label: {show: !0, radius: .75, formatter: function(e, t) {
                            return'<div style="font-size:8pt;text-align:center;padding:2px;color:white;">' + e + "<br/>" + Math.round(t.percent) + "%</div>"
                        }, background: {opacity: .5}}}}, legend: {show: !1}});
        $.plot($("#flot-8"), c, {series: {pie: {innerRadius: .5, show: !0}}})
    }
    if ($("#flot-4").length > 0) {
        var r = [];
        for (var i = 0; i <= 10; i += 1)
        ...
        var p = 0, d = !0, v = !1, m = !1;
        function g() {
            $.plot($("#flot-4"), [r, s, o], {series: {stack: p, lines: {show: v, fill: !0, steps: m}, bars: {show: d, barWidth: .6}}})
        }
        g();
    ...
    if ($("#flot-3").length > 0) {
        ...
        function b() {
            ...
            while (c.length < y) {
                var e = c.length > 0 ? c[c.length - 1] : 50, t = e + Math.random() * 10 - 5;
                t < 0 && (t = 0);
                t > 100 && (t = 100);
                c.push(t)
            }
        ...
        var E = {series: {shadowSize: 0}, yaxis: {min: 0, max: 100}, xaxis: {show: !1}}, l = $.plot($("#flot-3"), [b()], E);
        function S() {
            l.setData([b()]);
            l.draw();
            setTimeout(S, w)
    ...
    if ($("#flot-1").length > 0) {
        ...
        for (var i = 0; i < Math.PI * 2; i += .1)
            N.push([i, Math.tan(i)]);
        $.plot($("#flot-1"), [{label: "sin(x)", data: x}, {label: "cos(x)", data: T}, {label: "tan(x)", data: N}], {series: {lines: {show: !0}, points: {show: !0}}, xaxis: {ticks: [0, [Math.PI / 2, "π/2"], [Math.PI, "π"], [Math.PI * 3 / 2, "3π/2"], [Math.PI * 2, "2π"]]}, yaxis: {ticks: 10, min: -2, max: 2}, grid: {backgroundColor: {colors: ["#fff", "#fff"]}}})
    }
    if ($("#flot-audience").length > 0) {
        var c = [[1262304e6, 1300], [12649824e5, 2200], [12674016e5, 3600], [127008e7, 5200], [1272672e6, 4500], [12753504e5, 3900], [12779424e5, 3600], [12806208e5, 4600], [12832992e5, 5300], [12858912e5, 7100], [12885696e5, 7800], [12912417e5, 8195]];
        $.plot($("#flot-audience"), [{label: "Visits", data: c, color: "#3a8ce5"}], {xaxis: {min: (new Date(2009, 12, 1)).getTime(), max: (new Date(2010, 11, 2)).getTime(), mode: "time", tickSize: [1, "month"], monthNames: ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]}, series: {lines: {show: !0, fill: !0}, points: {show: !0}}, grid: {hoverable: !0, clickable: !0}, legend: {show: !1}});
    ...
    if ($("#flot-hdd").length > 0) {
        var c = [[1364598e6, 10], [13646016e5, 12], [13646052e5, 14], [13646088e5, 14], [13646124e5, 10], [1364616e6, 16], [13646196e5, 18], [13646232e5, 15], [13646268e5, 16], [13646304e5, 18], [1364634e6, 20], [13646376e5, 22], [13646412e5, 24], [13646448e5, 25], [13646484e5, 27], [1364652e6, 31], [13646556e5, 33], [13646592e5, 36], [13646628e5, 37], [13646664e5, 38], [136467e7, 39], [13646736e5, 42], [13646772e5, 45], [13646808e5, 47], [13646844e5, 50]];
        $.plot($("#flot-hdd"), [{label: "HDD usage", data: c, color: "#f36b6b"}], {xaxis: {min: (new Date("2013/03/30")).getTime(), max: (new Date("2013/03/31")).getTime(), mode: "time", tickSize: [3, "hour"]}, series: {lines: {show: !0, fill: !0}, points: {show: !0}}, grid: {hoverable: !0, clickable: !0}, legend: {show: !1}});
    ...
    $(".flot-line").length > 0 && $(function() {
        ...
        function o() {
            s.setData([{label: "CPU at %", data: n(), lines: {show: !0, fill: !0}, points: {show: !1}, color: "#fd6e58"}]);
            s.draw();
            setTimeout(o, r)
        }
        var e = [], t = 20, r = 500, i = {series: {shadowSize: 0}, yaxis: {min: 0, max: 100}, xaxis: {show: !1}}, s = $.plot($(".flot-line"), [{label: "CPU at %", data: n(), lines: {show: !1, fill: !0}, points: {show: !1}, color: "#fd6e58"}], i);
    ...
    if ((location.pathname == "/flat/" || location.pathname == "/flat" || location.pathname == "/flat/index.html") && location.host != "localhost" && $(window).width() > 767) {
        ...
        bootbox.confirm("Would you like to start the page guide? It will show you functions you could miss!", "No", "Yes", function(e) {
            e && $.pageguide(O, {events: {close: function() {
                        $.pageguide("unload")
                    }}}).open()
        })
    }
    if ($("#sico").length > 0) {
        ...
        $("#sico").select2({formatResult: M, formatSelection: M, escapeMarkup: function(e) {
                return e
            }})
    }
    if ($("#simg").length > 0) {
        function _(e) {
            return e.id ? "<img style='padding-right:10px;' src='images/demo/flags/" + e.id.toLowerCase() + ".gif'/>" + e.text : e.text
        }
        $("#simg").select2({formatResult: _, formatSelection: _, escapeMarkup: function(e) {
                return e
            }})
    }
    $("#map1").length > 0 && $("#map1").gmap3({map: {options: {center: [22.49156846196823, 89.75802349999992], zoom: 7}}});
    $("#map4").length > 0 && $("#map4").gmap3({map: {options: {center: [46.578498, 2.457275], zoom: 5}}, marker: {values: [{latLng: [48.8620722, 2.352047], data: "Paris !"}, {address: "86000 Poitiers, France", data: "Poitiers : great city !"}, {address: "66000 Perpignan, France", data: "Perpignan ! GO USAP !", options: {icon: "http://maps.google.com/mapfiles/marker_green.png"}}], events: {click: function(e, t, n) {
                    var r = $(this).gmap3("get"), i = $(this).gmap3({get: {name: "infowindow"}});
                    if (i) {
                        i.open(r, e);
                        i.setContent(n.data)
                    } else
                        $(this).gmap3({infowindow: {anchor: e, options: {content: n.data}}})
                }}}});
    if ($("#map2").length > 0) {
        var D = new Gmap3Menu($("#map2")), P, H, B;
        function j(e, t) {
            t ? H = e : B = e;
            I()
        }
        function F(e) {
            ...
            $("#map2").gmap3({marker: {latLng: P.latLng, options: {draggable: !0, icon: new google.maps.MarkerImage("http://maps.gstatic.com/mapfiles/icon_green" + (e ? "A" : "B") + ".png")}, tag: e ? "from" : "to", events: {dragend: function(t) {
                            j(t, e)
                        }}, callback: function(t) {
                        j(t, e)
                    }}})
        ...
        function I() {
            if (!H || !B)
                return;
            $("#map2").gmap3({getroute: {options: {origin: H.getPosition(), destination: B.getPosition(), travelMode: google.maps.DirectionsTravelMode.DRIVING}, callback: function(e) {
                        if (!e)
                            return;
        ...
        D.add("Direction from here", "itemA separator", function() {
            D.close();
            F(!0)
        });
